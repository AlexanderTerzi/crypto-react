...
const ItemsPerPage: React.FC = () => {
    ...
    const handlePerPageSubmit = (e: React.SyntheticEvent) => {
        e.preventDefault();
        const val = inputRef.current?.value;

        if (inputRef.current && val && val.length != 0) {
...
const Pagination: React.FC = () => {
    ...
    const totalNumber = Math.ceil(Number(totalPages) / Number(itemsPerPage)) || 10;
